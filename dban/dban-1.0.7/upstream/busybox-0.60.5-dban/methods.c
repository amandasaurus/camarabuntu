 ...
 */
	...
	{
		r = errno;
		dwipe_perror( r, "dwipe_dod522022m: read:" );
 ...
 */
	...
	{
		r = errno;
		dwipe_perror( r, "dwipe_dodshort: read:" );
 ...
 */
	...
	{
		r = errno;
		dwipe_perror( r, "dwipe_gutmann: read:" );
 ...
 */
	...
	if( s == NULL )
	{
		dwipe_perror( errno, "dwipe_ops2: malloc:" );
	...
	{
		r = errno;
		dwipe_perror( r, "dwipe_ops2: read:" );
 ...
 */
	...
	/* The PRNG state. */
	dwipe_entropy_t entropy = { DWIPE_KNOB_PRNG_STATE_LENGTH, NULL };
	...
	entropy.s = malloc( entropy.length );

	/* Check the memory allocation. */
	if( ! entropy.s )
	{
		dwipe_perror( errno, "dwipe_runmethod: malloc:" );
		dwipe_log( DWIPE_LOG_FATAL, "Unable to allocate memory for the entropy state buffer." );
		return -1;
	}
	...
	while( c->round_working < c->round_count )
	{
		...
		for( i = 0 ; i < c->pass_count ; i++ )
		{
			...
			else
			{
				/* Seed the PRNG. */
				c->pass_type = DWIPE_PASS_WRITE;
				r = read( c->entropy_fd, entropy.s, entropy.length );
				c->pass_type = DWIPE_PASS_NONE;
	
				/* Check the result. */
				if( r < 0 )
				{
					dwipe_perror( errno, "dwipe_runmethod: read:" );
				...
				if( dwipe_options.verify == DWIPE_VERIFY_ALL )
				{
					...
					/* Verify this pass. */
					c->pass_type = DWIPE_PASS_VERIFY;
					r = dwipe_random_verify( c, &entropy );
					c->pass_type = DWIPE_PASS_NONE;
